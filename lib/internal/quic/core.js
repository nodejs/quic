...
const {
  codes: {
    ...
    ERR_QUICSOCKET_UNBOUND, // eslint-disable-line no-unused-vars
    ERR_QUIC_TLS13_REQUIRED // eslint-disable-line no-unused-vars
  },
  errnoException,
...
const {
  ...
  constants: {
    AF_INET,
    AF_INET6,
    SSL_OP_ALL, // eslint-disable-line no-unused-vars
    SSL_OP_DONT_INSERT_EMPTY_FRAGMENTS, // eslint-disable-line no-unused-vars
    SSL_OP_SINGLE_ECDH_USE, // eslint-disable-line no-unused-vars
    SSL_OP_CIPHER_SERVER_PREFERENCE, // eslint-disable-line no-unused-vars
    SSL_OP_NO_ANTI_REPLAY, // eslint-disable-line no-unused-vars
    UV_EBADF,
    UV_UDP_IPV6ONLY,
    UV_UDP_REUSEADDR,
    TLS1_3_VERSION // eslint-disable-line no-unused-vars
  }
...
function connectAfterLookup(session, type, port, ipv6Only, err, ip) {
  ...
  if (ipv6Only)
    flags |= UV_UDP_IPV6ONLY;
  const handle =
    _createClientSession(this, type, ip, port || 0, flags, sc.context);
  if (!handle) {
