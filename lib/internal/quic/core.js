...
function onStreamClose(appErrorCode, finalSize) {
  this[owner_symbol][kDestroy](appErrorCode, finalSize);
}
...
class QuicStream extends Duplex {
  ...
  [kDestroy](appErrorCode, finalSize) {
    // If this thing is still readable or writable,
    // emit the 'abort' event immediately before
    // destruction.
    if (this.readable || this.writable)
      this.emit('abort', appErrorCode, finalSize | 0);
    this.destroy();
  }
  ...
  // Close the QuicStream abruptly.
  close(code) {
    // This should call the underlying shutdown_stream
    // API in ngtcp2. That will eventually lead to the
    // stream being closed and destroyed but that won't
    // happen right away.
    if (this.destroyed) {
      // eslint-disable-next-line no-restricted-syntax
