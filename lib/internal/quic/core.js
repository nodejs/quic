...
function onSessionHandshake(
  ...
  session[kState].maxPacketLength = maxPacketLength;
...
class QuicSocket extends EventEmitter {
  ...
  [kMaybeDestroy]() {
    const { sessions } = this[kState];
    debug(`Maybe close socket? size: ${sessions.size}`);
    if (sessions.size === 0) {
...
class QuicSession extends EventEmitter {
  ...
  [kRemoveStream](stream) {
    this[kState].streams.delete(stream.id);
  }
