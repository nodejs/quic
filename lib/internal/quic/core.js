...
function afterPreferredAddressLookup(transportParams, port, type, err, ip) {
  if (err) {
    this.destroy(err);
    return;
  }
  this[kContinueListen](transportParams, { ip, port, type });
}
...
class QuicSocket extends EventEmitter {
  ...
  listen(options, callback) {
    ...
    const doListen =
      continueListen.bind(
        this,
        validateTransportParams(options),
        this.#lookup);
...
class QuicClientSession extends QuicSession {
  ...
  [kHandshakePost]() {
    ...
    if (type === 'DH' && size < this.#minDHSize) {
      session.destroy(new ERR_TLS_DH_PARAM_SIZE(size));
      return false;
    }
    return true;
  }
  ...
  [kContinueConnect](type, ip) {
    let flags = this.#ipv6Only ? UV_UDP_IPV6ONLY : 0;
