...
function onStreamReset(appErrorCode, finalSize) {
  this[owner_symbol][kStreamReset](appErrorCode, finalSize);
}
...
class QuicStream extends Duplex {
  ...
  // Close the QuicStream abruptly. Unlike destroy(), which
  // causes the QuicStream to be immediately destroyed without
  // sending any additional data to the remote peer, calling
  // close will transmit STOP_SENDING and RESET_STREAM frames
  // to the remote peer as necessary before allowing the
  // QuicStream to be destroyed. Any data pending in the send
  // queue will be discarded without waiting for acknowledgement.
  close(code) {
  ...
  _destroy(error, callback) {
    ...
    // TODO(@jasnell): Destroy with an error code
    if (handle !== undefined)
      handle.destroy();
    callback(error);
  }
